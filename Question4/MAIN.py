...
Boys=['Avi','Abhi','Kunal','Shashak','Saurav','Ankit','Jadu','Jangbahadur','Akhilesh','Amar','Akbar','Saras','Lolo','Dange','Ali','Mohammad']
...
for i in range(0,2*k):
	s1=singleboy[i]+'broke up with'+singlegirl[i]
	commited.append(s1)

s1='time to patch up'
commitd.append(s1)

for item in singleboy:
	for b in B:
		if b.name==item:
			b.statuschange()
			break

for item in singlegirl:
	for g in G:
		if g.name==item:
			g.statuschange()
			break
...
for boy in B:
	for girl in G:
		if boy.pairing(girl) and girl.pairing(boy) and boy.currentstatus()=='single' and girl.currentstatus()=='single' :
			boy.statuschange()
			girl.statuschange()
			c=COUPLE(boy.name,boy.type,girl.type,girl.name,boy.budget,girl.maintbudget,boy.attraction,girl.attraction,boy.intelligent,girl.intelligent)
			couplesList.append(c)
			s1=boy.name+' will date '+girl.name
			commited.append(s1)
break
